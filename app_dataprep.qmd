...
save(chicago, catchment_areas, study_area_svi_estimates, tract_boundaries_geom, study_area_svi_estimates_geom, wellness_center_map, file="wellness_app_data.RData")
...
wellness_center_map <- leaflet(study_area_svi_estimates_geom %>% st_transform(4326)) %>%
  ...
  addMapPane("Catchment Areas", zIndex = 420) %>%
  ...
  addPolylines(
    ...
    opacity = 1,
    color = "darkred",
    group = "Catchment Areas",
    options = pathOptions(pane = "Catchment Areas")
  ) %>%
  addCircleMarkers(
  ...
  addLayersControl(
    overlayGroups = c("Wellness Center",
                      "Catchment Areas"),
